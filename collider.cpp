...
// Constructors
Collider::Collider(int layer, Sprite* sprite) : layer(layer)
{
...
Collider::Collider(int layer, float2 size) : layer(layer), size(size)
{
	//collisionSystem = scene->GetCollisionSystem();
};

Collider::Collider(int layer, Tmpl8::float2 size, Tmpl8::float2 offset) : layer(layer), size(size), offset(offset) {};
